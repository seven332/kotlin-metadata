 ...
 */
class BinaryVersion(private vararg val numbers: Int) {
    ...
    fun writeTo(stream: OutputStream) {
        ...
        dataOutput.writeInt(numbers.size)
        numbers.forEach { dataOutput.writeInt(it) }
    }
    ...
    companion object {
        ...
        fun parseFrom(stream: InputStream): BinaryVersion {
            val dataInput = DataInputStream(stream)
            return BinaryVersion(*(1..dataInput.readInt()).map { dataInput.readInt() }.toIntArray())
        }
    }
}
